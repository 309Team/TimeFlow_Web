      ...
      <el-drawer v-model="drawer_data" :visible.sync="drawer">
        ...
        <el-card v-for="item in drawer_data" :key="item.id" :index="item.id" class="drawer-card">
          <el-descriptions :title="item.name" direction="vertical" :column="3" border>
            ...
            <el-descriptions-item label="开始时间">{{ item.startTime.slice(0, 9) }} {{ item.startTime.slice(11, 19) }}</el-descriptions-item>
            <el-descriptions-item label="结束时间">{{ item.overTime.slice(0, 9) }} {{ item.startTime.slice(11, 19) }}</el-descriptions-item>
